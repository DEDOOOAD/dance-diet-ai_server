...
from schemas import PoseAnalysisRequest, PoseAnalysisResponse, PosePoint
...
def calculate_movement_score(
    current_landmarks: Sequence[PosePoint],
    previous_landmarks: Sequence[PosePoint],
) -> float:
...
def analyze_pose_request(request: PoseAnalysisRequest) -> PoseAnalysisResponse:
    movement_score = calculate_movement_score(
        request.current_landmarks,
        request.previous_landmarks,
    )
